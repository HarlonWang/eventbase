...
前缀由消费方通过 `basePath` 传给库（首个消费方用 `/t` 与 `/t/q`），库内部用 Hono 的 `basePath` 处理——消费方直接 `.fetch()`，**不必自己依赖 hono 做前缀嫁接**。摄取与取数是两个 factory：`createIngest` 与 `createQuery`，可分别挂载或只挂其一。取数面默认不发 CORS 头；浏览器页面要直读时由消费方传 `corsOrigins`（origin 精确白名单），预检先于鉴权放行，正式请求仍要 token。
...
```jsonc
{
  ...
  "user": "identity-id",         // 可选，登录后带上；服务端可配 identitySecret 以 HMAC 假名落库，原值不进 D1
...
同一张表，`source='server'`，不经 HTTP：消费方在自己的 Worker 里用 `createTracker(db)` 直接写 D1。地理六项（`country` / `asn` / `colo` / `timezone` / `city` / `region`）取自 `request.cf`，因此 writer 必须能拿到请求上下文；客户端摄取路径落同样六项。
