...
| S5 | `new_installs?by=sys_locale` → 500（列不存在） | ultra | ✅ 已修 |
| S6 | `retention` 忽略 `by` | ultra | ✅ 已修 |
| S7 | `retention` 不过滤 `is_debug`，与 `active` 口径不一致 | ultra | ✅ 已修 |
| S8 | `events:[null]` 崩溃，破坏恒 204 契约 | ultra | ✅ 已修 |
| S9 | 文档写 24 小时、代码是 7 天 | CR | ✅ 已修（改文档） |
| S10 | 两处测试未用 try/finally 恢复现场 | CR | ✅ 已修 |
| **S11** | 配额先扣后写，写失败不补偿 → 重试再扣 → 触顶后 204 **永久丢批** | CR | ⬜ **P0** |
| **S12** | `first_day` 用 `receivedAt` 而事件用 `event_at` → 离线补报跨日时 cohort 与事件错位，retention 失真 | CR | ⬜ **P0** |
...
S1~S10 由另一个会话自主修复（2026-08-19，43 测试通过）。
...
1. 两个仓加 `.coderabbit.yaml`：docstring 门禁只对**导出符号**生效，内部实现豁免——与 CLAUDE.md 的注释准入对齐（80% 全量覆盖率的默认门禁与「注释只留反直觉」直接冲突）；
2. 客户端 P0 八条 → 一个分支 + PR（K2/K3 合并改，K1 用 Channel 串行化）；
3. 客户端 P1 → 一个分支 + PR；
4. 服务端 S11/S12 + P1 → 等另一会话那批改动落盘后再动，避免冲突；
