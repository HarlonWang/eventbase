...
| `ai_requested` | `kind`（chat；~~detail_summary~~ 2026-09 退役，「一键解读」功能下线、后续改预生成不经客户端 AI 请求；~~research~~ 2026-09 退役，Deep Research 功能下线——41 天 9 次使用、2/3 来自已删的升级入口，数据见 research_runs 表）, `from`, `image_count`；~~has_context~~ 2026-09 随入口上下文机制下线退役（chat 唯一入口为通用入口，值恒空） | `chat_send` / `detail_summary_generate` / `research_start` |
