...
**载体**：各 App 自己的 Cloudflare D1；取数走带 token 的 HTTP 接口。v1 不做看板 UI；2026-09-23 起有浏览器看板套件，见 `dashboard.md`。

**明确不在范围**：

| 不做 | 理由 |
|---|---|
| 看板 / 图表 UI（v1） | v1 的消费者是 SQL 与 Claude，不是眼睛；后已补上，见 `dashboard.md` |
